...
# just verifying that the sample c program I wrote to test fftw works as expected
template = np.arange(0, 16)
signal = np.cos(template) + 1j*np.sin(template)
output = np.fft.fft(signal)
for i in range(16):
    print(f"{np.abs(output[i])} exp(i{np.angle(output[i])})")
